...
*** Pinned outputs
The fixture run's deterministic outputs are pinned by md5 in =tests/expected_outputs.md5= and compared by =tools/check_expected_outputs.sh tests/full=, which prints =ok= or =DIFF= per file and exits 1 on any difference; =--write= regenerates the pins. A =.gz= file is summed on its decompressed bytes. Only files that were byte-identical across two clean fixture runs are pinned (32 files): the per-bin counts, fragment BEDs and GC distributions, ratios, length histograms, end-motif counts and matrices, the derived features, and =ref/keep_5mb.bed=. Left out: the filtered BAMs (their headers differ between runs), the three QC PDFs and the two fastp HTML reports (both differed between runs), the fastp JSON and processed FASTQs (their content depends on fastp's threading and the paths it records), the aligned BAMs and the BWA index (large binaries downstream steps already cover), and =healthy_med.rds= (R serialization format depends on the R version).
