...
type: object
required: [conda, directories, sample-tsv-path, frag_ref_assemblies, delfi_bins, blklist, cytoband]
properties:
  conda:
    type: object
    required: [frag]
...
*** Root Snakefile
The root =Snakefile= runs the fixture wrapper, so =snakemake --configfile config/test.yaml= from the repository root runs the test pipeline.
...
def read_counts(path):
    ...
    d = pd.read_csv(path, sep="\t", header=None, names=["motif", "count"], index_col="motif")
...
def blacklisted_bases(blacklist, bed_path, tmpdir):
    ...
    merged = subprocess.run(
        ["bedtools", "merge", "-i", blk_path], check=True, capture_output=True, text=True
    ).stdout
...
def arms(cytoband, bins):
    ...
    p_end = cyto[cyto["band"].astype(str).str.startswith("p")].groupby("chr")["end"].max()
...
CASES = {
    ...
    "r1_minus": (pair("r1_minus", 83, 201, 163, 61), [minus_motif(201), plus_motif(61)]),
    ...
    "mate_unmapped": ([rec("mu", 73, 51, pnext=51), rec("mu", 133, 51, mapq=0, cigar="*", pnext=51)], []),
    "mapq29": (pair("lowq", 99, 71, 147, 171, mapq=29), []),
    # QC-fail (0x200) reads are kept, as in the reference implementation.
    "qcfail": (pair("qcf", 99 + 512, 131, 147 + 512, 260), [plus_motif(131), minus_motif(260)]),
    # Soft clip: the end is the first aligned base, not the clipped read start.
    "softclip": (pair("sc", 99, 101, 147, 250, cigar1="5S25M"), [plus_motif(101), minus_motif(250)]),
...
def run_script(tmp_path, bam, fa, max_ends=0, seed=42, name="out.tsv"):
    out = tmp_path / name
    subprocess.run(
        ["bash", str(SCRIPT), str(bam), str(fa), str(out), "1", str(max_ends), str(seed)],
        check=True,
    )
...
def test_read1_minus_takes_fragment_5prime_end(tmp_path):
    ...
    assert not interior & set(true_ends), "fixture seed makes an interior motif equal a true end"
...
def test_max_ends_non_integer_fails(tmp_path):
    ...
    res = subprocess.run(
        ["bash", str(SCRIPT), str(bam), str(fa), str(tmp_path / "x.tsv"), "1", "ten", "42"],
...
def test_library_with_no_ends_fails(tmp_path):
    ...
    res = subprocess.run(
        [sys.executable, str(SCRIPT), str(tmp_path / "c.tsv"), str(tmp_path / "f.tsv"), str(p)],
        capture_output=True, text=True,
    )
    assert res.returncode != 0
...
NAME = re.compile(r"\b(D_[A-Z_]+|FRAG_[A-Z_]+|CONDA_[A-Z_]+|NMF_[A-Z_]+|frag_ref_names)\b")
